...
class Home extends CI_Controller {
    ...
    function crear_menu_nivel_1($sistema, $perfil, $idioma) {
        ...
        if (!$padre) {
            ...
            foreach ($padre as $row) {          
                $hijo = $this->Menus->menu_hijo($row->id);
                $item = "";
                if (!$hijo) {
                    if ($idioma == "es"){
                    ...
                    $item = $item . "<ul>";
                    $item = $item . $this->crear_menu_sub_niveles($row->id, $idioma);
                    $item = $item . "</ul>";
                    $item = $item . "</li>";
                }
                $nivel1 = $nivel1 . $item;
    ...
    function crear_menu_sub_niveles($id_item, $idioma) {
        $masnivel = "";
        
        $hijos = $this->Menus->menu_hijo($id_item);
        if (!$hijos) {
            return "";
        }
        else{
            foreach ($hijos as $row) {          
                $masHijos = $this->Menus->menu_hijo($row->id);
                $item = "";
                if (!$masHijos) {
                    if ($idioma == "es"){
                        $item = $item . "<li><a href='" . $row->url . "'>" . $row->icono . $row->texto . "</a></li>";
